...
if (str (_display) != "no display") then
{
	...
	_ChildControl  ctrlSetTooltip format ["Cost: %1 ой",server getVariable (SDKEng select 0)];
	_ChildControl = _display displayCtrl 108;
	_ChildControl  ctrlSetTooltip format ["Cost: %1 ой",server getVariable (SDKExp select 0)];
