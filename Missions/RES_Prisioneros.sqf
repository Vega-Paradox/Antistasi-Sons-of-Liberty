...
[[buenos,civilian],"RES",[format ["A group of POWs is going to be executed in %1. We must rescue them before %2:%3. Bring them back to our HQ.",_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"POW Rescue",_marcador],_posicion,false,0,true,"run",true] call BIS_fnc_taskCreate;
...
if ({alive _x} count _POWs == 0) then
	{
	["RES",[format ["A group of POWs is going to be executed in %1. We must rescue them before %2:%3. Bring them back to our HQ.",_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"POW Rescue",_marcador],_posicion,"FAILED","run"] call A3A_fnc_taskUpdate;
	...
	{
	sleep 5;
	["RES",[format ["A group of POWs is going to be executed in %1. We must rescue them before %2:%3. Bring them back to our HQ.",_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"POW Rescue",_marcador],_posicion,"SUCCEEDED","run"] call A3A_fnc_taskUpdate;
